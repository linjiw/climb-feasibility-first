...
| **Phase-G seal draft — seal-ready, awaiting approval** | `PREREGISTRATION_G_SEGMENT.md` remains unsealed. Endpoint-blind calibration selected `rho = 0.40`, `lambda = 0.05`; the independent seed passed (mean TV 0.1056, minimum 700.1 effective units, maximum top-1 0.0134, zero invalid/censored events). This is a manipulation result only, not policy benefit. The 512-env footprint is measured, all 900 payload identities pass, and contact timing is explicitly frozen exploratory-only because independent human labels are absent. The compact calibration decision includes all 39 hash-bound selector ledgers and excludes multi-gigabyte checkpoints/exports. The pre-seal checklist is closed, but the advisor directive forbids unilateral confirmatory runs; no seal or confirmation launch occurs without explicit approval. | `plan/PREREGISTRATION_G_SEGMENT.md`, `plan/G2_CALIBRATION_RESULT_2026-09-04.md`, `reports/g_segment/calibration/result.json`, `plan/G_CONTACT_TIMING_DISPOSITION_2026-09-04.md` |
