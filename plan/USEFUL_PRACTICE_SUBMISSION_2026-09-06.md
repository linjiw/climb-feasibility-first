...
Natural-failure retirement now passes on four development attempts, with an
independent replay of both failed and successful runs and 14 new integrity tests.
A reference-only audit authenticates all 100 held-out motion payloads but locates
no corresponding frame-level admission records in the report tree. Full C1
selection awaits authenticated screen reconstruction; clip-level feasibility
cannot stand in for continuous admission. Current result and exact next actions:
