...
def prepare(out: Path, report_root: Path) -> dict:
    ...
    contract["entrypoint_smokes"] = {arm: pending_or_record(report_root / "confirmation_entrypoint_smokes" / f"{arm}_result.json")
                                    for arm in ("U", "A", "R", "D")}
    ...
    write_once(out / "entrypoint_smoke_preview.json", {"seed": 51, "num_envs": 8, "iterations": 20,
                "after": "fixed D calibration", "jobs": [
                    {"arm": arm, "argv": [str(ROOT / "mjlab-1.6.0/.venv/bin/python"),
                      ...
                      "--out-dir", str(report_root / "confirmation_entrypoint_smokes" / arm)]}
                    for arm in ("U", "A", "R", "D")]})
...
if __name__ == "__main__":
    ...
    print(json.dumps(prepare(args.out_dir.resolve(), args.report_root.resolve()), indent=2))
