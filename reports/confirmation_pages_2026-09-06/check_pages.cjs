...
(async () => {
  ...
  for (const theme of ['light','dark']) for (const width of [390,1440]) {
    ...
    for (const file of ['index.html','relative-progress.html','archive-2026-09-05.html','flagship.html','companion.html','segment-native.html']) {
      ...
      if(['index.html','relative-progress.html'].includes(file)){
        await page.screenshot({path:`reports/confirmation_pages_2026-09-06/${file.replace('.html','')}_${width}_${theme}.png`});
        if(width===1440&&theme==='light')await page.locator(file==='index.html'?'#results':'#evidence').screenshot({path:`reports/confirmation_pages_2026-09-06/${file.replace('.html','')}_evidence.png`});
      }
  ...
  assert.match(await nojs.locator('body').innerText(),/12 \/ 12/);
  await browser.close();
  fs.writeFileSync('reports/confirmation_pages_2026-09-06/browser_checks.json',JSON.stringify({checks:results,keyboard_interaction:'pass',no_javascript_fallback:'pass'},null,2)+'\n');
