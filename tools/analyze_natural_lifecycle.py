#!/usr/bin/env python3
"""Verify natural termination, mixed-horizon scoring and isolated reset histories."""
...
def verify_lifecycle(data: dict, rows: list[dict], conditions: list[dict], name: str) -> dict:
    ...
    last_reset = [0] * count
    boundaries = {r["physics_index"]: r["ids"].tolist() for r in resets}
...
def analyze(run: Path) -> dict:
    ...
    return {"status": "natural_lifecycle_pass" if covered else "natural_lifecycle_valid_but_coverage_incomplete",
