#!/usr/bin/env python3
"""N1 -- knee-contact inverse dynamics of a reference clip (CPU, MuJoCo).
...
def latest_model_xml():
    ...
    if not xs:
        raise SystemExit("no compiled G1 xml; run the S1 harness once")
    return xs[-1]
...
def main():
    ...
    ap.add_argument("--clip", default="BMLmovi_Subject_64_F_MoSh_Subject_64_F_9_poses_120_jpos")
    ...
    m = mujoco.MjModel.from_xml_path(latest_model_xml())
    d = mujoco.MjData(m)
    m.opt.disableflags |= mujoco.mjtDisableBit.mjDSBL_CONTACT   # contact-free ID
    dat = np.load(os.path.join(BANK, args.clip + ".npz"))
    ...
    def binsum(model_name):
        ...
        return res
    summary = {"clip": args.clip, "fps": fps, "gap": args.gap, "mu": args.mu, "model": latest_model_xml(),
