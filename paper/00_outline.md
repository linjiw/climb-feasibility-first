...
| 10 | Limitations and scope — one robot; one reward configuration; survival-centric endpoints; 4k-iteration horizon (Exp-1/2); sim-only; LUCID-correlation small N; explicit descoping of the solver-ensemble program with reasons | slot (Aug 23–Sept 5) | `flagship/S10_limitations.md` |
...
| F2 | collapse mechanism timeline: entropy/top-1 vs held-out gap | **to write** `paper/figures/f2_collapse.py` | `reports/A5_coverage_dose.json`, `reports/campaign_summary_3arm.json` |
...
| F4 | prevalence by category × source | **to write** `paper/figures/f4_prevalence.py` | `reports/feasibility_all/feasibility.csv` |
| F5 | transfer lift (intrinsic vs +support vs +feasibility, perm baselines) | **to write** `paper/figures/f5_transfer.py` | `reports/N2_atlas_support.json`, `reports/N_atlas_v21.json` |
