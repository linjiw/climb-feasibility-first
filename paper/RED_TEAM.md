...
| 17 | Repair census 65.8 % (payoff §2, companion §8) | success budget circular (repair judged by the same screen); smoothing hides new artifacts | budget includes *independent* terms (≤15 cm geometry, added-velocity); no-op-on-control + ballistics-refusal are external checks; residual velocity spikes acknowledged as unscreened | ✅ wording in A3 |
| 18 | Saturation-at-fall (5/29 at ≥98 %) | saturation an artifact of termination thrash, not impact | pre-fall saturation exactly 0.0 in 8/8; onset within 0.6 s of contact event; per-actuator identity (wrists/waist) consistent with landing loads | ✅ (`effort_sat_at_fall.json`) |
| 19 | E-HYG concentration signature (future) | worst-decile regression-to-mean mimics concentration | decile defined on the *comparator* arm, deltas evaluated on the pruned arm (independent draws); permutation test over clips; noted for the readout | 🕐 at readout |
