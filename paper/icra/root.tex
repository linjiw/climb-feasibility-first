...
Screening Reference--Physics Misalignment and Testing Adaptive Allocation\\
on Exact Support for Humanoid Motion Tracking
}
...
\texttt{infeasible\_frac}$>0.10$ (strict inequality). A frame whose torque-limited program is
...
Equation~\eqref{eq:gate} is the general rule at exploration ratio 0.4 and relative factor 2.
...
\texttt{whole\_body\_tracking} to G1. We report raw counts under the fixed rule and
...
features with and without the three \texttt{refeas} features against 200 control designs that
...
Deployment / exploratory & One policy, 26 clips (22 qualified repairs plus 4 byte-identical
...
$\Delta=-0.0101$; one seed pair, permutation over held-out feasible clips, $p=0.951$. \\
