...
`infeasible_frac`$>0.10$ (strict inequality). Ballistic flight is not flagged merely for
...
`whole_body_tracking` to G1. We report raw counts under the fixed rule, category/source
...
features with and without the three `refeas` features against 200 random three-feature
...
| --- | --- |
| E-HYG / sealed | Whole-clip pruning: feasible-panel survival $0.918\to0.907$; $\Delta=-0.0101$, one-sided permutation $p=0.951$. |
